...
```typescript
import { apiClient, ApiError, isApiError, getErrorMessage, formatErrorForDisplay } from '@/lib/api';
...
```typescript
import { formatErrorForDisplay } from '@/lib/api';
...
├── dtos.ts             # Data Transfer Objects matching NestJS API
...
1. Add DTOs to `dtos.ts`
2. Create or update resource file in `resources/`
