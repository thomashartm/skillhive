...
export const GET = wrapDb(
  async (AppDataSource, _request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    try {
      ...
      if (video.techniqueId) {
        ...
        categories = categories.filter((c) => c !== null) as any[];
      }
...
export const PUT = wrapDb(
  async (AppDataSource, request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    try {
      ...
      const { ReferenceAsset } = await import('@trainhive/db');
